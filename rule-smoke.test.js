...
assert.deepEqual(branchCodes("cross-treatment", "root-traction"), ["013105010330000"]);
assert.ok(!treatments.find(item => item.id === "occlusal-function").branches.some(item => item.id === "root-traction"));
...
assert.deepEqual(branchCodes("root-canal", "perforation-repair"), ["013306020030000"]);
...
assert.match(treatments.find(item => item.id === "extraction").name, /阻生牙/);
assert.match(catalog.find(item => item.n === 75).name, /腭部/);
assert.match(catalog.find(item => item.n === 76).name, /腭部/);
assert.match(catalog.find(item => item.n === 67).note, /完成1个疗程计价收费1次/);
assert.ok(catalog.find(item => item.n === 107).conflicts.includes(catalog.find(item => item.n === 38).code));
assert.ok(catalog.find(item => item.n === 38).conflicts.includes(catalog.find(item => item.n === 107).code));
assert.equal(routine.phases[0].items.find(item => item.code === "013105010050001").when.ageMax, 6);
assert.equal(treatments.find(item => item.id === "root-canal").branches.find(item => item.id === "foreign").phases[0].items.find(item => item.code === "013105010090001").when.foreignLocation, "apical");
